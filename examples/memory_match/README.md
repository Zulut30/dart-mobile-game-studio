...
- **Real lifecycle path** — explicit pause/resume/menu transitions, app-background auto-pause, and
  no mismatch timer left running behind a paused overlay.
- **Responsive shell** — constraint-driven 2/3/4/6/8-column boards, bounded tablet width, safe
  scrolling, and widget tests for compact and wide layouts.
...
lib/
  ...
  systems/                   PURE Dart — the rules
    ...
    game_logic.dart          flip / resolveMismatch / pause / resume / menu reducer
  widgets/                   thin Flutter UI
    app.dart  menu_screen.dart  game_screen.dart  card_tile.dart  status_bar.dart
test/
  game_state_test.dart  board_factory_test.dart  game_logic_test.dart
  widget_test.dart  accessibility_test.dart
```
...
dart test test/board_factory_test.dart test/game_logic_test.dart test/game_state_test.dart
flutter test                    # core + widget + lifecycle + accessibility tests
flutter run                     # play it on a device/emulator
```

CI (`.github/workflows/ci.yml`, the `example` job) runs a separate Dart-VM core gate before
`flutter test`. The weekly [`release-canary`](../../.github/workflows/release-canary.yml) also creates
disposable Android/iOS runners and builds an AAB plus an unsigned iOS release app.
