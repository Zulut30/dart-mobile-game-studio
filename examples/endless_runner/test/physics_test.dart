...
void main() {
  ...
  group('Physics.clampDt', () {
    test('clamps a hitch to the max step', () {
      ...
      expect(Physics.clampDt(double.nan), 0);
      expect(Physics.clampDt(double.infinity), 0);
    });
  });

  group('Physics.integrate', () {
    test('a jump rises, gravity falls', () {
      final rising = Physics.integrate(
        const Runner(vy: 760, grounded: false),
        1 / 60,
        c,
      );
      expect(rising.y, greaterThan(0));
      final falling = Physics.integrate(
        const Runner(y: 100, grounded: false),
        1 / 60,
        c,
      );
    ...
    test('landing snaps to the ground and re-grounds', () {
      final landed = Physics.integrate(
        const Runner(y: 0.001, vy: -1000, grounded: false),
        1 / 60,
        c,
      );
      expect(landed.y, 0);
